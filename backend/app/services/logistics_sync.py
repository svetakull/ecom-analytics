...
def process_financial_report(
    ...
    for row in rows:
        ...
        # Объём из финотчёта (литры) — для справки
        vol_report = float(row.get("delivery_amount", 0) or 0)
        ...
        # Источник 1: Объём по карточке продавца (Content API)
        card = card_dims.get(nm_id)
        vol_card = float(card.volume_liters) if card else 0.0

        # Источник 2: Объём по номенклатуре WB (отчёт номенклатур, загруженный пользователем)
