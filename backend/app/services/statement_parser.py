...
CLASSIFICATION_RULES = [
    ...
    # === Склад Калмыкия (ИП Фальковский) — всегда склад, даже если в назначении «фулфилмент» ===
    (r"(?i)фальковский|ИП.*ФАЛЬКОВСКИЙ", "warehouse_kalmykia", 0.97),
    ...
    (r"(?i)чуприна.*евгени|ИП\s*Чуприна", "ff", 0.95),  # ИП Чуприна — ФФ
    ...
    (r"(?i)ROSTELECOM|ростелеком", "pvz_video", 0.9),  # видеонаблюдение ПВЗ
    (r"(?i)MEGASET|мегасет", "pvz_internet", 0.9),  # интернет ПВЗ
    ...
    (r"(?i)вложени.*инвестор|взнос.*учредител|пополнение.*счет.*учредител", "investor_contribution", 0.9),  # вложения инвестора
    (r"(?i)выдача.*кредит|зачислени.*кредит|выборка.*кредит|транш.*кредит|кредитн.*лини.*выборк", "credit_received", 0.9),  # получение кредита
    ...
    (r"(?i)обращени.*с.*твёрд.*коммунальн|\bтко\b|коммунальн.*отход", "pvz_tko", 0.9),  # ТБО/ТКО
