...
CLASSIFICATION_RULES = [
    ...
    (r"(?i)\bРВБ\b|ООО.*РВБ", "mp_payment", 0.95),  # поступление от ВБ
    (r"(?i)купишуз|kupishuz", "mp_payment", 0.95),  # поступление от Lamoda
    (r"(?i)интернет.?технологии|ООО.*интернет.*технолог", "mp_payment", 0.95),  # поступление от Ozon
    (r"(?i)АО.*ТБанк|тинькофф|tinkoff|финансов.*и.*платёжн.*технолог|финансовые и платежные технологии", "mp_payment", 0.9),  # поступление от сайта
    (r"(?i)ГУРИНА\s+ЯНА|гурина.*олеговна", "mp_payment", 0.9),  # оптовые продажи
    (r"(?i)улюмжуев|ИП.*Улюмжуев", "pvz", 0.9),  # аренда ПВЗ
    ...
    # === Назначение платежа ===
    (r"(?i)зарплат[аы]|з/?п\b|фот\b|оплата труда|дневн.*уход.*за.*ребёнк|дневного ухода за ребенком", "salary_manager", 0.9),
    ...
    (r"(?i)ндфл", "ndfl", 0.85),
    (r"(?i)таможен|таможня|customs", "purchase_china", 0.85),  # таможенные платежи
    ...
    (r"(?i)реклам[аы]|яндекс.*директ|google.*ads|vk.*ads|таргет", "external_ads", 0.8),
...
def classify_entries(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        if category in ("_transfer", "_transfer_in", "_transfer_out"):
            ...
            entry_type = "income"
            category = "other"  # доход от депозита
        elif category == "mp_payment":
            entry_type = "income"  # поступления от МП всегда доход
        else:
